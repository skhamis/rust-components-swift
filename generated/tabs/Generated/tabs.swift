...
private extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_tabs_14ee_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_tabs_14ee_rustbuffer_free(self, $0) }
    }
}
...
public class TabsStore: TabsStoreProtocol {
    ...
    public convenience init() {
        self.init(unsafeFromRawPointer: try!
            ...
            rustCall {
                tabs_14ee_TabsStore_new($0)
            })
    }

    deinit {
        try! rustCall { ffi_tabs_14ee_TabsStore_object_free(pointer, $0) }
    }

    public func getAll() -> [ClientRemoteTabs] {
        let _retval = try!
            rustCall {
                tabs_14ee_TabsStore_get_all(self.pointer, $0)
            }
    ...
    public func setLocalTabs(remoteTabs: [RemoteTab]) {
        try!
            rustCall {
                tabs_14ee_TabsStore_set_local_tabs(self.pointer, FfiConverterSequenceRecordRemoteTab.lower(remoteTabs), $0)
            }
    }
    ...
    public func registerWithSyncManager() {
        try!
            rustCall {
                tabs_14ee_TabsStore_register_with_sync_manager(self.pointer, $0)
            }
    }
}
...
private extension RemoteTab {
    ...
    func write(into buf: Writer) {
        ...
        FfiConverterSequenceString.write(urlHistory, into: buf)
...
private extension ClientRemoteTabs {
    static func read(from buf: Reader) throws -> ClientRemoteTabs {
        return try ClientRemoteTabs(
            ...
            remoteTabs: FfiConverterSequenceRecordRemoteTab.read(from: buf)
        )
    }
    ...
    func write(into buf: Writer) {
        ...
        FfiConverterSequenceRecordRemoteTab.write(remoteTabs, into: buf)
    }
}
...
// Helper code for RemoteTab record is found in RecordTemplate.swift
...
private enum FfiConverterSequenceRecordClientRemoteTabs: FfiConverterUsingByteBuffer {
    ...
    static func read(from buf: Reader) throws -> SwiftType {
        try FfiConverterSequence.read(from: buf) { buf in
            try ClientRemoteTabs.read(from: buf)
        }
    }
}

private enum FfiConverterSequenceRecordRemoteTab: FfiConverterUsingByteBuffer {
    typealias SwiftType = [RemoteTab]
